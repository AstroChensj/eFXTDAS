"""Logging helpers for FXT extraction-region generation."""
...
def _default_logger() -> logging.Logger:
    """Return a process-local default stream logger for fxtregions."""
    logger = logging.getLogger("eFXTDAS.fxtregions.default")
    if logger.handlers:
        return logger
...
def build_cli_logger(name: str, level: str | None, logfile: str | Path | None = None) -> logging.Logger:
    ...
    if not any(isinstance(handler, logging.StreamHandler) and not isinstance(handler, logging.FileHandler) for handler in logger.handlers):
        _ensure_handler(logger, logging.StreamHandler(), level=resolved_level)
...
def emit(logger: logging.Logger | None, level: str, message: str) -> None:
    ...
    logger : logging.Logger | None
        Logger used for structured messages. When ``None``, messages are
        printed directly.
    level : str
        Logging level name such as ``"info"`` or ``"warning"``.
    message : str
        Message to emit.
