...
def _apply_env_overrides(prefix: str) -> None:
    ...
    for name, current in list(globals().items()):
        ...
        try:
            ...
            raise ValueError(
                f"Invalid override for fxtcombine constant {name} via {env_name}: "
...
SRC_EXTRACT_RADIUS = 60         # source extraction radius [arcsec]
BKG_EXTRACT_INNER_RADIUS = 90   # background extraction inner radius [arcsec]
BKG_EXTRACT_OUTER_RADIUS = 300  # background extraction outer radius [arcsec]
INSTBKG_BACKSCAL_RELSTD_WARN = 0.1


_apply_env_overrides("FXTCOMBINE_")
